from models import tournoi, rounds, match
from controllers import controllerjoueur
from views import viewtournoi
...
class ControllerTournoi:
    def __init__(self):
        ...
        - current_tournament (obj): contient le tournoi
        sélectionné ou créé (initialisé en None).
        - current_round (obj): contient le round en cours du tournoi
        sélectionné ou créé (initialisé en None).
        """
        self.tournament_model = tournoi.Tournoi
        self.round_model = rounds.Round
        self.match_model = match.Match
        self.tournament_view = viewtournoi.ViewTournoi(self)
        self.player_controller = controllerjoueur.ControllerJoueur()
        self.current_tournament = None
        self.current_round = None

    def create_new_tournament(self, name, location, description):
        """Crée un nouveau tournoi, l'enregistre dans la base de données et
        le sélectionne comme tournoi par défaut."""
    ...
    def add_players(self):
        """Ajoute les joueurs de la salle d'attente à la liste
    ...
    def get_match_result(self):
        ...
        for contest in self.current_round.match_list:
            print(f"Match >>> Joueur 1 : {contest.joueur1} "
                  f"-- VS -- Joueur 2 : {contest.joueur2}")
            result = self.tournament_view.get_match_result()
            if result == "1":
                winner = contest.joueur1
    ...
    def load_existing_tournament(self, tournament_name):
        """Charge un tournoi existant puis sélectionne
        le tournoi et son dernier round."""
        existing_tournament = self.tournament_model.\
            create_from_json(tournament_name)
        self.current_tournament = existing_tournament
        self.current_round = self.current_tournament.rounds_list[-1] \
            if self.current_tournament.rounds_list else None

    def begin_tournament(self):
        """
        Commence le tournoi. Initialise le numéro de round,
        mélange les joueurs et crée les paires puis
        sauvegarde le tout.
        """
        self.add_players()
        self.current_tournament.save_json_file()
    ...
    def create_next_round(self):
        ...
        Crée le prochain round, l'ajoute à la liste
        de rounds du tournoi puis le sélectionne.
    ...
    def get_existing_tournaments(self):
        """Retourne une liste des tournois qui sont sous forme
        de fichier JSON dans la base de données."""
