import os
from controllers import controllertournoi, controllerrapport


NUMBER_OF_PLAYERS_IN_TOURNAMENT = 8
tournament_controller = controllertournoi.ControllerTournoi()
...
report_controller = controllerrapport.ControllerRapport()
...
while running:
    ...
    main_menu_list = [f"[1] > Ajouter un joueur - Salle d'attente : "
                      f"{len(player_controller.waiting_room)} / "
                      f"{NUMBER_OF_PLAYERS_IN_TOURNAMENT} joueurs",
    ...
    if user_choice == "1":
        ...
        if os.path.exists("data/rounds/"):
            ...
            print("Impossible d'accéder au menu Rapport"
                  "car la base de données est vide !")
    elif user_choice == "4":
        print("Fermeture du programme !")
        quit()
    else:
        print()
        print(f"Erreur. Veuillez choisir un numéro"
              f"entre 1 et {len(main_menu_list)}.")
        print()
