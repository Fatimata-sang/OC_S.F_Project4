...
class Round:
    def __init__(self, round_name):
        ...
        Les attributs start_date et end_date seront renseignés avec
        l'horodatage de la date et de l'heure.
    ...
    def __repr__(self):
        """Définit le nom du round en tant que
        représentation de l'objet Round."""
        return f"{self.round_name}"

    @classmethod
    def create_from_json(cls, round_name):
        """Charge un objet round à partir d'un fichier
        JSON existant selon le nom du round."""
            ...
            with open(existing_json_file_path, "r",
                ...
                for match_data in round_data.get("match_list", []):
                    ...
                    match_object = match.Match.\
                        create_from_json(f"{player1_first_name} "
                                         f"{player1_name}",
                                         f"{player2_first_name} "
                                         f"{player2_name}")
