...
import os
import random
from models import match, rounds, joueur
...
class Tournoi:
        ...
        if os.path.exists(existing_json_file_path):
            with open(existing_json_file_path, "r", encoding="utf-8") as json_file:
                ...
                for participant in tournament_data.get("players_list", []):
                    ...
                    participant_object = joueur.Joueur. create_from_json(participant_name, participant_first_name)
    ...
    def add_player_to_tournament(self, player_to_add):
        """Ajoute le joueur sélectionné dans
        la liste des joueurs participants au tournoi."""
    ...
    def create_first_round_matches(self, first_round):
        ...
        for participant in range(0, len(self.pairs_list), 2):
            try:
                first_round.add_match(match.Match
                                      (self.pairs_list[participant],
                                       self.pairs_list[participant + 1]))
            except IndexError:
                return

    def create_next_round_matches(self, next_round):
        """Crée les matchs du round suivant selon
        les scores des joueurs par ordre décroissant."""
        sorted_players = sorted(self.players_list, key=lambda participants: self.players_scores[
            participants.national_chess_id],
                                reverse=True)
    ...
    def sort_players(self):
        ...
        temporary_ranking = [(participant, self.players_scores[participant.national_chess_id])
                             for participant in self.players_list]
        sorted_ranking = sorted(temporary_ranking,
    ...
    def initialize_players_scores(self):
        """Initialise le score des joueurs à zéro dans
        le cadre du tournoi."""
        players_id = []
        for participant in self.players_list:
    ...
    def add_one_point_to(self, participant):
        """Ajoute la valeur ci-dessous au score joueur
        sélectionné dans le cadre du tournoi."""
    ...
    def list_existing_tournaments():
        """Renvoie la liste des tournois présents dans
        la base de données par ordre alphabétique."""
        ...
        for tournament in os.listdir(directory_path):
            ...
            if tournament.endswith(".json") and os.path.isfile(file_names):
                existing_tournaments.append(tournament.replace("_", " ").
                                            replace(".json", ""))
    ...
    def to_dict(self):
        ...
        data = {
            ...
            "rounds_list": [r.to_dict() for r in self.rounds_list]
            if self.rounds_list else [],
            "players_list": [participant.to_dict() for participant
                             in self.players_list]
            if self.players_list else [],
            "pairs_list": [participant.to_dict() for participant
                           in self.pairs_list] if self.pairs_list else [],
            "players_scores": [
