...
class Joueur:
    def __init__(self, name, first_name, birthdate, national_chess_id):
        """Initialise un joueur avec son nom, prénom,
        date de naissance et identifiant national d'échec."""
        self.name = name
    ...
    def __repr__(self):
        ...
        en tant que représentation de l'objet Player."""
        return f"{self.first_name} {self.name}"

    @classmethod
    def create_from_json(cls, name, first_name):
        """Charge un objet joueur à partir d'un fichier
        JSON existant selon le nom et prénom du joueur."""
    ...
    def list_existing_players():
        """Renvoie une liste classée par ordre alphabétique de tous
        les joueurs existants dans la base de données."""
        existing_players = []
        directory_path = "data/players/"
        ...
        for file in os.listdir(directory_path):
            ...
            if file.endswith(".json") and os.path.isfile(file_name):
                ...
                created_player = Joueur.create_from_json(split_player_name[1],
                                                         split_player_name[0])
                existing_players.append(created_player)

        return sorted(existing_players, key=lambda participant:
