...
class Match:
    def __init__(self, player1, player2):
        ...
        En fonction de l'issue du match,
        l'attribut winner contiendra le gagnant du match
    ...
    def __repr__(self):
        """Définit la composition du match en
        tant que représentation de l'objet Match."""
    ...
    def create_from_json(cls, player1, player2):
        ...
        Les paramètres sont le joueur 1 et
        le joueur 2 du match que l'on souhaite charger.
        """
        existing_json_file_path = f"data/matches/{player1}_VS_{player2}.json"
        if os.path.exists(existing_json_file_path):
            with open(existing_json_file_path, "r",
                      encoding="utf-8") as json_file:
                match_data = json.load(json_file)
                player1_data = match_data["player1"]
                player1_name = player1_data["name"]
                player1_first_name = player1_data["first_name"]
                player1_object = joueur.Joueur.\
                    create_from_json(player1_name, player1_first_name)
                player2_data = match_data["player2"]
                player2_name = player2_data["name"]
                player2_first_name = player2_data["first_name"]
                player2_object = joueur.Joueur.\
                    create_from_json(player2_name, player2_first_name)
                winner_data = match_data["winner"]
                winner_name = winner_data["name"] \
                    if winner_data else None
                winner_first_name = winner_data["first_name"] \
                    if winner_data else None
                winner_object = joueur.Joueur.\
                    create_from_json(winner_name, winner_first_name) \
                    if winner_data else None
                ...
                for player_data in match_data.get("draw", []):
                    ...
                    player_object = joueur.Joueur.\
                        create_from_json(player_name, player_first_name)
    ...
    def is_draw(self):
        ...
        self.draw = [self.joueur1, self.joueur2]
    ...
    def update_json_file(self):
        ...
        json_file_name = f"data/matches/{self.joueur1}_VS_{self.joueur2}.json"
    ...
    def to_dict(self):
        ...
        data = {
            "player1": self.joueur1.to_dict(),
            "player2": self.joueur2.to_dict(),
            "winner": self.winner.to_dict() if self.winner else None,
            "draw": [participant.to_dict() for participant in self.draw]
            if self.draw else []
        }
        return data
